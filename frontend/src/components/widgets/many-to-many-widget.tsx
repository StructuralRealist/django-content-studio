...
export function ManyToManyWidget({
  ...
  return (
    <Popover modal open={open} onOpenChange={setOpen}>
      <PopoverTrigger className="w-full flex items-center gap-1 text-left border hover:border-stone-300 rounded-md px-3 h-8 text-sm select-none">
        <div className="flex-1 flex items-center gap-1">
          {value.map((item: any) => (
            <Badge key={item.id}>{item.__str__}</Badge>
          ))}
        </div>
